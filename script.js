// SAMBELIX — Mobile-first loader (Google Sheet) dengan UX rapi
document.addEventListener('DOMContentLoaded', () => {
  /* Performa & adaptasi: perbaiki 100vh iOS */
  ...
  /* Sumber data (kecepatan & fleksibel): isi salah satu */
  const PUBLISHED_TSV_URL = ""; // opsional: "Publikasikan ke web" (output=tsv) → paling stabil
  const SHARE_LINK = "https://docs.google.com/spreadsheets/d/10bjcfNHBP6jCnLE87pgk5rXgVS8Qwyu8hc-LXCkdqEE/edit?usp=drivesdk"; // link kamu sekarang
  ...
  /* NAV mobile: manusiawi (mudah dibuka/ditutup, fokus jelas) */
  ...
  /* URL builder: fleksibel (publish TSV atau share link) */
  ...
  function render(items, animate=false){
    ...
    items.forEach((it, idx)=>{
      ...
      card.innerHTML = `
        <img class="card__img" src="${media}" alt="${esc(it['Nama Menu'])}" loading="lazy" decoding="async"
      ...
      setTimeout(()=>{ // animasi halus (stagger kecil)
